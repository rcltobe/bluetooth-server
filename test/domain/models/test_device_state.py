...
class TestDeviceState(unittest.TestCase):
    def test_should_update_state(self):
        ...
        state = DeviceState(address, False)

        # 状態が変化したときのみ、更新する。
        self.assertTrue(state.should_update_state(DeviceState(address, True)))
        self.assertFalse(state.should_update_state(DeviceState(address, False)))
